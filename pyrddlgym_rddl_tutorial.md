...
The first command uses two parameters after the `.rddl` files to specify (1) the number of trials to run and (2) the time limit for PROST. The second command takes a single parameter, which is the path to the JSON file generated by the first command.
